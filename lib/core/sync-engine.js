...
import { emptyState, mergeCommit, removeCommit, } from "./config.js";
...
async function doPull(args) {
    ...
    const commit = manifest.commits.find((c) => c.hash === manifest.head);
    if (commit === undefined) {
        return { report: errorReport(lines, '上游清单里找不到最新提交对应的文件'), state, manifest };
    }
    return await pullCommit({ deps, manifest, commit, state, lines, say });
}
/**
 ...
 */
export async function restoreCommit(deps) {
    const { targetHash, ...rest } = deps;
    const { transport, config } = rest;
    const lines = [];
    const say = (line) => {
        lines.push(line);
        rest.onLine?.(line);
    ...
    try {
        ...
        return { report: errorReport(lines, `读取上游清单失败：${describe(error)}`), state: rest.state, manifest: null };
    }
    const commit = manifest.commits.find((c) => c.hash === targetHash);
    if (commit === undefined) {
        return { report: errorReport(lines, `上游清单里找不到该历史版本：${targetHash.slice(0, 12)}`), state: rest.state, manifest };
    }
    say(`恢复历史版本：${targetHash.slice(0, 12)}（${commit.file}）`);
...
export async function deleteCommit(deps) {
...
async function pullCommit(args) {
    const { deps, manifest, commit, state, say, lines } = args;
    ...
    try {
        ...
        return {
            ...
            state: {
                ...state,
                lastSyncedHash: head,
                // 恢复之后本机内容 = 这一版内容，两边锚点对齐到同一个 hash
                lastLocalHash: head,
