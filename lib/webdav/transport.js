...
/** 默认请求实现：node:http/https，支持任意方法、准确 Content-Length、自动跟随重定向。 */
const defaultRequest = async (method, url, options, auth) => {
    let currentMethod = method;
    let currentUrl = url;
    let currentHeaders = { ...(options.headers ?? {}) };
    let currentBody = options.body;
    if (auth !== null)
        currentHeaders['authorization'] = auth;
    for (let redirects = 0;; redirects++) {
        const res = await rawRequest(currentMethod, currentUrl, currentHeaders, currentBody, options.timeoutMs ?? DEFAULT_TIMEOUT_MS);
        if (!REDIRECT_STATUSES.has(res.status) || res.headers['location'] === undefined)
            return res;
...
function rawRequest(method, url, headers, body, timeoutMs) {
...
export class WebdavTransport {
    ...
    constructor(options) {
        ...
        if (base === '')
            throw new WebdavError('WebDAV 地址为空');
        this.base = base;
    ...
    /** 覆盖式写入一个文件。 */
    async put(file, body, contentType = 'application/octet-stream') {
 ...
 */
export function parsePropfind(xml, dirUrl) {
    const out = [];
...
export function isSameDir(href, dirUrl) {
