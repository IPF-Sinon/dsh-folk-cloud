...
function retryDelayMs(retryAfter, attempt) {
    ...
    if (Number.isFinite(secs) && secs >= 0)
        return Math.min(Math.max(secs * 1000, RETRY_BASE_MS), RETRY_MAX_WAIT_MS);
    const at = Date.parse(retryAfter);
    if (!Number.isNaN(at))
        return Math.min(Math.max(at - Date.now(), RETRY_BASE_MS), RETRY_MAX_WAIT_MS);
    return fallback;
}
...
function rawRequest(method, url, headers, body, timeoutMs, onUploadProgress) {
...
export class WebdavTransport {
    ...
    async put(file, body, contentType = 'application/octet-stream', onProgress) {
