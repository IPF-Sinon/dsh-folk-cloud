...
export async function readBridgeConfig(): Promise<BridgeConfig | null> {
  try {
    const raw = await fs.readFile(APP_BRIDGE_CONFIG, 'utf8');
    const parsed = JSON.parse(raw) as { port?: unknown; token?: unknown };
    if (typeof parsed.port !== 'number' || typeof parsed.token !== 'string' || parsed.token === '') return null;
...
function request(
  ...
  method: string,
  pathname: string,
  options: { token?: string; body?: Buffer; contentType?: string; timeoutMs: number },
): Promise<{ status: number; body: Buffer }> {
  return new Promise((resolve, reject) => {
    const headers: Record<string, string> = { accept: 'application/json' };
    if (options.token !== undefined) headers['x-dsh-fs-token'] = options.token;
    if (options.body !== undefined) {
      headers['content-type'] = options.contentType ?? 'application/octet-stream';
      headers['content-length'] = String(options.body.length);
    }
    const req = http.request(
      { method, host: '127.0.0.1', port, path: pathname, headers },
      (res) => {
        const chunks: Buffer[] = [];
...
): Promise<{ status: number; json: Record<string, unknown> | null; raw: Buffer }> {
  const payload = body === undefined ? undefined : Buffer.from(JSON.stringify(body), 'utf8');
  const port = await managerPort();
  if (port === null) throw new Error('找不到 DSH web 服务端口（dsh 可能没在运行）');
 ...
 */
async function managerPort(): Promise<number | null> {
...
/** 宿主 App 的补包接口是否可用。 */
export async function appBridgeAvailable(): Promise<boolean> {
  ...
  try {
    ...
    const parsed: unknown = JSON.parse(res.body.toString('utf8'));
    return typeof parsed === 'object' && parsed !== null && (parsed as { available?: unknown }).available === true;
  } catch {
    return false;
  }
}
 ...
 */
export class HostBackupProducer implements BackupProducer {
  async produce(outDir: string, tier: BackupTier, options: ProduceOptions): Promise<ProducedBackup> {
    ...
    if (wantsApp) return await this.produceWithApp(outDir, tier, options);
    return await this.produceWithoutApp(outDir, tier, options);
  }

  /** 含软件数据：整包交给宿主 App 产出（复用它自己的合并/加密通路）。 */
  private async produceWithApp(outDir: string, tier: BackupTier, options: ProduceOptions): Promise<ProducedBackup> {
    const bridge = await readBridgeConfig();
  ...
  private async produceWithoutApp(outDir: string, tier: BackupTier, options: ProduceOptions): Promise<ProducedBackup> {
    if (!tierHasDsh(tier)) throw new Error(`档位 ${tier} 需要宿主 App 支持，但接口不可用`);
    ...
    if (options.includeSessions) body['only'] = undefined;
    ...
    if (zipPath === '') throw new Error('dsh-config-manager 没有返回 zipPath');
...
export class HostBackupRestorer implements BackupRestorer {
  async restore(file: string, options: RestoreOptions): Promise<RestoreReport> {
    const bridge = await readBridgeConfig();
    if (bridge === null) {
    ...
    const body = Buffer.from(
      JSON.stringify({ file, password: options.password ?? '' }),
      'utf8',
    );
