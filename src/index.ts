...
import { isLoopbackRequest, readJsonBody, writeJson } from './core/http.ts';
import {
  HostBackupProducer,
  HostBackupRestorer,
  appBridgeAvailable,
...
class CloudRuntime {
  ...
  async config(): Promise<CloudConfig> {
  ...
  async saveConfig(raw: Record<string, unknown>): Promise<CloudConfig> {
  ...
  async forget(): Promise<CloudState> {
    const state: CloudState = { ...(await this.state()), lastSyncedHash: '', lastLocalHash: '' };
  ...
  reschedule(cfg: CloudConfig): void {
    ...
    if (minutes < MIN_INTERVAL_MINUTES) return;
...
function fallbackTierOf(tier: CloudConfig['tier']): CloudConfig['tier'] {
  if (tier === 'app-dsh-vault') return 'dsh-vault';
  if (tier === 'app-dsh') return 'dsh-only';
  if (tier === 'app-only') return 'dsh-only';
  return tier;
}
 ...
 */
export function apply(ctx: Context, config?: { dataDir?: string }): void {
...
export function makeRoutes(runtime: CloudRuntime): WebRoute[] {
